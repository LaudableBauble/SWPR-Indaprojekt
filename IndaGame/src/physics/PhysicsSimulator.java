...
/**
 * A physics simulator does just what its name implies; it tries to simulate the
 * physics of bodies so that the game world behaves realistically.
 */
public class PhysicsSimulator {
	 ...
	 * @return Whether the two bodies are close enough to warrant a possible
	 *         collision.
	 */
	private boolean broadPhase(Body b1, Body b2) {
		...
		try {
			...
			return Vector2.getDistance(b1.getPosition(), b2.getPosition()) < (Math
					.max(b1.getShape().getWidth(), b1.getShape().getHeight()) + Math
					.max(b2.getShape().getWidth(), b2.getShape().getHeight()));
		}
		// Catch the exception and display relevant information.
		catch (Exception e) {
			System.out.println(this + ": Broad Phase Error. (" + e
					+ ") - Body1: " + b1 + " Body2: " + b2);
		}
	...
	/**
	 * Do a narrow phase collision check between two bodies by using SAT
	 * (Separating Axis Theorem). If a collision has occurred, get the MTV
	 * (Minimum Translation Vector) of the two intersecting bodies.
	 * 
	 * @param b1
	 *            The first body to check.
	 * @param b2
	 *            The second body to check.
	 * @return The MTV of the intersection or null if the collision was
	 *         negative.
	 */
	public Vector2 narrowPhase(Body b1, Body b2) {
		...
		Vector2[][] axes = new Vector2[][] { b1.getShape().getAxes(),
				b2.getShape().getAxes() };
			...
			for (Vector2 a : v) {
				...
				if (o == -1) {
					...
					if (o < overlap) {
						...
						// projected upon.
						overlap = o;
						smallest = (Vector2.subtract(b1.getShape().getCenter(),
								b2.getShape().getCenter()).dot(a) < 0) ? Vector2
								.multiply(a, -1) : a;
					}
				}
			}
	...
	/**
	 * Pull two bodies that are intersecting apart by exerting forces. Old as of
	 * just now.
	 * 
	 * @param b1
	 *            The first body.
	...
	private void clearIntersection(Body b1, Body b2) {
		...
		Vector2 b1Pos = Vector2.getDirection(Vector2.getAngle(collision,
				b1.getPosition()));
		Vector2 b2Pos = Vector2.getDirection(Vector2.getAngle(collision,
				b2.getPosition()));
		// Multiply the direction with the absolute velocity of the body.
		Vector2 b1Force = Vector2.multiply(b1Pos,
				Vector2.absolute(b1.getVelocity()));
		Vector2 b2Force = Vector2.multiply(b2Pos,
				Vector2.absolute(b2.getVelocity()));
	...
	private void clearIntersection(Body b1, Body b2, Vector2 mtv) {
		...
		// move dynamic bodies.
		if (!b1.getIsStatic()) {
			b1.getShape().setPosition(
					Vector2.add(b1.getShape().getPosition(), mtv));
		}
		if (!b2.getIsStatic()) {
			b2.getShape().setPosition(
					Vector2.subtract(b2.getShape().getPosition(), mtv));
		}

		System.out.println(this + ": Clear Intersection. (MTV: " + mtv
				+ ", Body1: " + b1.getPosition() + ", Body2: "
				+ b2.getPosition() + ")");
	}
	...
	private Force impactForce(Body b1, Body b2) {
		...
		Vector2 energyB1 = Vector2.absolute(Vector2.multiply(b1.getVelocity(),
				b1.getMass()));
		Vector2 energyB2 = Vector2.absolute(Vector2.multiply(b2.getVelocity(),
				b2.getMass()));
		...
		Vector2 averageEnergy = Vector2.multiply(
				Vector2.multiply(Vector2.divide(energyBT, 2), energyDecrease),
				massRatio);

		// Multiply the Average Kinetic Energy with the collision vector
		// direction relative to the body's position.
		Vector2 impact = Vector2.multiply(
				averageEnergy,
				Vector2.getDirection(Vector2.getAngle(collision,
						b1.getPosition())));
	...
	private Force impactForceEnergy(Body b1, Body b2) {
		...
		Vector2 energyB1 = Vector2.divide(
				Vector2.multiply(
						Vector2.absolute(Vector2.multiply(b1.getVelocity(),
								b1.getVelocity())), b1.getMass()), 2);
		Vector2 energyB2 = Vector2.divide(
				Vector2.multiply(
						Vector2.absolute(Vector2.multiply(b2.getVelocity(),
								b2.getVelocity())), b2.getMass()), 2);
		...
		// collision force.
		Vector2 averageEnergy = Vector2.multiply(
				Vector2.divide(Vector2.add(energyB1, energyB2), 2),
				energyDecrease);
		...
		if (!bodyForces.isEmpty()) {
			...
			try {
				...
				for (Force f : bodyForces) {
					...
					f.getBody()
							.setVelocity(
									Vector2.add(f.getBody().getVelocity(),
											f.getForce()));
				}
			}
			// Catch the exception.
	...
	private Force getBodyFriction(Body b) {
		...
		return (new Force(b, Vector2.multiply(
				Vector2.inverse(Vector2.getDirection(b.getVelocity(),
						Vector2.getLength(Vector2.absolute(b.getVelocity())))),
	...
	private void addFrictionForce(Force f) {
		...
		try {
			...
			Vector2 friction = Vector2.add(f.getBody().getVelocity(),
					f.getForce());
			// ////////////////////////////////////////////////////////
			...
			// velocity.
			f.getBody().setVelocity(
					Vector2.clam(f.getBody().getVelocity(), friction, 0));
			// ////////////////////////////////////////////////////////
		...
		// Catch the exception.
			...
			System.out.println(this
					+ ": Adding Friction Force to Body Error. (" + e + ")");
		}
	}
}